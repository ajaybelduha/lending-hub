...
export const createPipelineContent = (props, values, formValues) => {
    let tagId = 4305274
      ...
      if (props.type === types.CREDITCARD) {
        ...
        if (formValues.category === CardCategories.PERSONAL) {
          subTag = PIPELINE_ID.personal_tag;
        } else if (formValues.category === CardCategories.STUDENT) {
          subTag = PIPELINE_ID.student_tag;
        } else {
          subTag = PIPELINE_ID.business_tag;
        }
      } else if (props.type === types.MORTGAGE) {
        tagId = PIPELINE_ID.mortgage_tag;
        if (formValues.mortgageType === MortgageTypes.HOME_BUYING) {
          subTag = PIPELINE_ID.home_buying;
